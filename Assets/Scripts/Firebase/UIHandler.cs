...
namespace Firebase.Sample.Auth {
  ...
  public class UIHandler : MonoBehaviour {
    ...
    protected void DisplayDetailedUserInfo(Firebase.Auth.FirebaseUser user, int indentLevel) {
            Debug.LogError("--- Dispaly DetailedUserInfo isverified=" + user.IsEmailVerified);
      string indent = new String(' ', indentLevel * 2);
    ...
    void AuthStateChanged(object sender, System.EventArgs eventArgs) {
      ...
      if (senderAuth == auth && senderAuth.CurrentUser != user) {
        ...
        if (signedIn) {
          DebugLog("AuthStateChanged Signed in " + user.UserId+"::isverified="+user.IsEmailVerified);
